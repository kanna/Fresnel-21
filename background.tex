...
\cite{marta012}. % (refs pex Marta Almeida et al
% 2012) 
...
These challenges are even more expressive in biogeochemical (BGC)
...
response to upwelling favorable winds with and \kc{(refs Hickey,
  )}. In the particular case of long submarine canyons that cut the
...
\begin{figure}[!h]
  ...
  the placement of the two buoys.]{\label{fig:domain}\includegraphics[scale=0.5]{fig/domain.jpg}}
  \caption{\subref{fig:po-map} \& \subref{fig:domain} show detailed
    views of the proposed study area for \proje.}
...
have showed \kc{citation?} that these contributions \kc{``combine in a
  fresh and high turbidity'' -- unclear} that can extend its influence
over the complete shelf \kc{(include photo)}.
