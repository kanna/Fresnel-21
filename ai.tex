\subsection{AI \& Robotics in \proje}

\begin{figure}[!h]
  \centering 
  \subfigure[]{\label{fig:vehicles}\includegraphics[scale=0.07]{fig/vehicles.jpg}}
  \hspace{+0.5cm} 
  \subfigure[]{\label{fig:agent}\includegraphics[scale=0.07]{fig/TREX-1.jpg}}
  \subfigure[]{\label{fig:components}\includegraphics[scale=0.25]{fig/TREX-auv.pdf}}
  \caption{\subref{fig:vehicles} shows autonomous aerial and
    ...
    specific functional and temporal scope.  \subref{fig:components}
    shows the hierarchy of architectural components for \proj which
    consists of an instance of \rx with an embedded planner. Within
    each \eu reactor, resides a plan database and search engine, which
    comprise of representational primitives for
...
dynamically incorporating plan repair \cite{rajan12,rajan12b}. The
symbolic planner at the heart of \rx has a rich legacy from \nas
...
(SCM) using Gaussian Process models \cite{fossum19}.
